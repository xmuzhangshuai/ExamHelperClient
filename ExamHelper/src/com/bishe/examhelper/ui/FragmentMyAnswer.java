...
/**
 * 类名称：FragmentMyAnswer
 * 类描述：我回答过的疑问
 * 创建人： 张帅
 * 创建时间：2014-4-17 上午10:32:34
 *
 */
public class FragmentMyAnswer extends BaseV4Fragment {
	...
	private class MyQueryListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				view = LayoutInflater.from(getActivity()).inflate(R.layout.my_answer_list_item, null);
				...
				holder.linearLayout = (LinearLayout) view.findViewById(R.id.myAnswerView);
			...
			String jsonString = (String) netData.get(position).get("answerList");

			List<JAnswerQuery> jAnswerQueries = FastJsonTool.getObjectList(jsonString, JAnswerQuery.class);
			holder.linearLayout.removeAllViews();
			if (jAnswerQueries != null) {

				for (JAnswerQuery jAnswerQuery : jAnswerQueries) {
					View answerlistView = getActivity().getLayoutInflater().inflate(R.layout.answer_list_item_view,
							null);
					TextView myAnswer = (TextView) answerlistView.findViewById(R.id.myAnswerContent);
					TextView myAnswerTime = (TextView) answerlistView.findViewById(R.id.myAnswerTime);
					myAnswer.setText(jAnswerQuery.getAnswer_content());
					myAnswerTime.setText(DateTimeTools.getInterval(jAnswerQuery.getAnswer_time()));
					holder.linearLayout.addView(answerlistView);
				}
			}
